...
import { Search, Sparkles, BookOpen, Play, Clock, Cpu, X, FileJson, Settings, BarChart3 } from 'lucide-react';
import { McqDeck } from '../../types/mcq';

interface CommandPaletteProps {
  isOpen: boolean;
  onClose: () => void;
  onNavigate: (view: 'studio' | 'library' | 'quiz' | 'analytics' | 'import') => void;
  onStartQuiz: (deck: McqDeck, mode: 'practice' | 'exam' | 'flashcard') => void;
  onTrimMemory: () => void;
  onOpenSettings?: () => void;
  decks: McqDeck[];
}
...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  const filteredDecks = decks.filter((d) => {
    ...
    const questionMatch = d.questions && d.questions.some((q) => q.topic?.toLowerCase().includes(qLower));
    const cardMatch =
      d.cards &&
      d.cards.some(
        (c) =>
    ...
    return titleMatch || questionMatch || cardMatch;
  });
      ...
      >
        ...
        <div className="max-h-80 overflow-y-auto p-2 space-y-1">
          ...
          >
            ...
            <div className="flex-1">
              <div className="font-medium text-zinc-200 group-hover:text-[#10B981]">Analytics & Mastery Hub</div>
              <div className="text-[11px] text-zinc-400">View test accuracy, retention metrics, and study history</div>
            </div>
            <span className="text-[10px] font-mono text-zinc-500">Jump</span>
          </button>
                  ...
                  >
                    <div className="flex items-center gap-2.5 truncate mr-3">
                      ...
                      <div className="truncate">
                        ...
                        <div className="text-[10px] text-zinc-400 font-mono">
                          {isFlashcard
                            ? `${deck.cards?.length || 0} theory cards • Flashcards`
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                      {isFlashcard ? (
                        <button
                          onClick={() => {
                            onStartQuiz(deck, 'flashcard');
                            onClose();
                          }}
                          className="px-2.5 py-1 bg-[#10B981] text-[#09090B] hover:bg-[#059669] text-[10px] flex items-center gap-1 font-semibold transition-colors"
                        >
                          <Play className="w-3 h-3" /> Study Flashcards
                        </button>
                      ) : (
                        <>
                          <button
